...
use super::{SecureTextFieldDelegate, SECURE_TEXTFIELD_DELEGATE_PTR};
...
extern "C" fn text_did_end_editing<T: SecureTextFieldDelegate>(this: &Object, _: Sel, _info: id) {
    let view = load::<T>(this, SECURE_TEXTFIELD_DELEGATE_PTR);
    let s = NSString::retain(unsafe { msg_send![this, stringValue] });
    view.text_did_end_editing(s.to_str());
}

extern "C" fn text_did_begin_editing<T: SecureTextFieldDelegate>(this: &Object, _: Sel, _info: id) {
    let view = load::<T>(this, SECURE_TEXTFIELD_DELEGATE_PTR);
    let s = NSString::retain(unsafe { msg_send![this, stringValue] });
    view.text_did_begin_editing(s.to_str());
}

extern "C" fn text_did_change<T: SecureTextFieldDelegate>(this: &Object, _: Sel, _info: id) {
    let view = load::<T>(this, SECURE_TEXTFIELD_DELEGATE_PTR);
    let s = NSString::retain(unsafe { msg_send![this, stringValue] });
    view.text_did_change(s.to_str());
}

extern "C" fn text_should_begin_editing<T: SecureTextFieldDelegate>(this: &Object, _: Sel, _info: id) -> Bool {
    let view = load::<T>(this, SECURE_TEXTFIELD_DELEGATE_PTR);
    let s = NSString::retain(unsafe { msg_send![this, stringValue] });
...
extern "C" fn text_should_end_editing<T: SecureTextFieldDelegate>(this: &Object, _: Sel, _info: id) -> Bool {
    let view = load::<T>(this, SECURE_TEXTFIELD_DELEGATE_PTR);
    let s = NSString::retain(unsafe { msg_send![this, stringValue] });
...
/// need to do.
pub(crate) fn register_view_class_with_delegate<T: SecureTextFieldDelegate>(instance: &T) -> &'static Class {
    load_or_register_class("NSSecureTextField", instance.subclass_name(), |decl| unsafe {
        // A pointer to the "view controller" on the Rust side. It's expected that this doesn't
        // move.
        decl.add_ivar::<usize>(SECURE_TEXTFIELD_DELEGATE_PTR);
